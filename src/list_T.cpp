...
void List<T>::removeAtIndex(const int index)
{
	...
	while (ctr != index)
	{ // find the node that's at the index position
		prev = curs;
		curs = curs->next();
		ctr++;
	}
	if (curs->next() == NULL)
	{					   // check if it's the last node on the list
	...
	{
		if (start == curs)
		{ // check if it's the first node on the list
			start = curs->next();
		}
		else
		{
...
void List<T>::push(T *v)
{
	...
	else
	{
		...
		{ // move until we either find the end of the list
			curs = curs->next();
		}
...
int List<T>::indexOf(const T v) const
{
	...
	while (curs != NULL && !found)
	{ // move until we match or get to the end of the list
		if (*(curs->content()) == v)
		{
			found = true;
		}
