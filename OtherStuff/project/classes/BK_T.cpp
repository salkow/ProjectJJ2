...
template <typename T>
BK<T>::Node::Node(T *& content, int parentDistance) :
	content(content), parentDistance(parentDistance){};
...
List<T> BK<T>::recSearch(Node * parent, T * query, int maxDistance) const {
	...
	if (distanceFunction(parent->content, query) <= maxDistance) results.push(parent->content);
